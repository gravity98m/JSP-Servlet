...
public class Test3 {
	...
	public static void main(String[] args) {
		...
		// 기본형 타입(boolean(1) char(2) byte(1) short(2) int(4) long(8) float(4) double(8))
		// 형변환 	1) 묵시적 형변환(자동) : 메모리 크기 작은 곳 -> 큰 곳 갈 때 발생
		//					=> 컴파일러가 자동으로 형변환 실행(에러 없음)
		//			2) 명시적 형변환(강제) : 메모리 크기 큰 곳 -> 작은 곳 갈 때 발생
		...
		// 형변환	1) 업캐스팅 (자동 형변환) : 슈퍼클래스의 레퍼런스가 서브클래스의 인스턴스를 가리키는 것
		// 										부모타입으로 형변환
		// 										서브클래스의 레퍼런스 값이 슈퍼클래스의 레퍼런스 변수에 저장
		// 				
		// 			2) 다운캐스팅 (강제 형변환) : 서브클래스의 레퍼런스가 슈퍼클래스의 인스턴스를 가리키는 것
		//										자식타입으로 형변환
...
//		sc2.pprn();
		...
		// => 실행 시 (runtime 에러) 없는 영역에 참조가능성 있기 때문에 예외 발생
