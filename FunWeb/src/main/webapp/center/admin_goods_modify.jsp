...
<body>
	<div id="wrap">
		...
		<article>
		<h1>관리자 상품 수정페이지</h1>
			<form action="./AdminGoodsModifyPro.ag" method="post">	 <!-- 관리자가 첨부파일을 수정하기해보기 -->
				<input type="hidden" name="gno" value="${dto.gno }"> <!-- 삭제(delete)와 새로운 것(update) 업로드 -->
			<table id="notice">										 <!-- 수정정보 업로드 > 삭제 > 수정 -->
				<tr>						
					<th class="ttitle" colspan="2">ITWILL 상품 수정</th>
				</tr>
				<tr>
					<td>카테고리</td>
					<td>
						<select name="category" >
							...
							>정장</option>
							<option value="Tshirt"
								<c:if test="${dto.category.equals('Tshirt') }">
									selected="selected"
								</c:if>
							>티셔츠</option>
							<option value="shirt" 
								<c:if test="${dto.category.equals('shirt') }">
									selected="selected"
								</c:if>
							>셔츠</option>
							<option value="pants"
